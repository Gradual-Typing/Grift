...
(provide parse)
#| Some type shorthand to keep types from getting huge as they do|#
...
(: env-lookup (Env Symbol Src . -> . Uvar))
...
(: syntax-unroll (Stx . -> . (U Symbol String CL (Listof Stx))))
(define (syntax-unroll stx)
  (let ((a (syntax-e stx)))
    (if ((make-predicate (U Symbol CL String (Listof Stx)))
	 a)
	a
...
#| The parser |#
(: parse-expr (Stx Env Natural . -> . (values CF Natural)))
(define (parse-expr stx env next)
  (let ((src (syntax->srcloc stx))
	...
	(cond
     ...
     [(Core-Literal? exp) (values (Quote exp src) next)] 
     [else (raise-unsupported-syntax-exn stx)])))

(: parse-variable (Symbol Src Env . -> . CF))
...
(define-type Acc2 
  (List (Listof CFml) (Listof Symbol) Env Natural))
...
(define-type Acc1 (Pair (Listof CBnd) Natural))
...
(: parse-primitive (Core-Prim (Listof Stx) Src Env Natural . -> . 
			      (values CF Natural)))
...
(: parse-expr* ((Listof Stx) Env Natural . -> . (values (Listof CF) Natural)))
...
(: parse-top-level
   (String (Listof Stx) Env Natural . -> . (values CF Natural)))
