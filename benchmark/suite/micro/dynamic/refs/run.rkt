...
(module+ main
  ...
  (command-line
   ...
   #:args ()
   (dynamic-write-read-plot-and-latex!
    (run-dynamic-write-reads-benchmark
     (iterations-parameter)
     (runs-parameter)
     (epsilon-parameter)))))

(define (run-dynamic-write-reads-benchmark iterations number-of-runs epsilon)
...
(define (dynamic-write-read-plot-and-latex! brs)
      ...
      (cond
        ...
        [else
         (match-define (list mean-overhead _)
           (benchmark-results-stats-ref brss 'overhead (list c d l)))
...
(define (make-exe-path test compiler dyn-ops? dyn-loop?)
  (build-path
   ...
   (case compiler
     ...
     [(twosomes coercions)
      (define dynloop (if dyn-loop? ".dynloop" ""))
      (define dynops  (if dyn-ops?  ".dynops" ""))
      (format "~a.~a~a~a" test compiler dynops dynloop)])))

(define (make-src-path test compiler dynamic-loop?)
  (build-path
   src-dir
   (case compiler
