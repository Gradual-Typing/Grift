...
(require/typed/provide
 ...
 [check (case-lambda
          ...
          ((Any Any -> Any) Any Any String -> Any))]
...
(require (prefix-in s: racket/base))
...
#;(define-rewriter t:test-begin test-begin
  ...
  [s:synstax-case syntax-case])

(define-type Check-Exceptional (U (Rec flat
                                       (U Boolean Complex Char
                                          Null Symbol String
                                          Keyword (Pairof flat flat)))
                                  exn))

(define-predicate exceptional? Check-Exceptional)

(: check-around ((Thunk Any) -> Any))
(define (check-around thunk)
  (with-handlers ([exceptional? (current-check-handler)]
                  [(lambda a #t) (lambda a (raise "whoops"))])
    (thunk)))
...
(require/typed/provide
 ...
 [current-check-handler
  (Parameter (-> Check-Exceptional Any))]
 [current-check-around
  (Parameter ((Thunk Any) -> Any))])

;; Andre inlining the text-ui and trying to patch up his test suite
