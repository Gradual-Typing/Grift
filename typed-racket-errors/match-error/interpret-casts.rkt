    ...
    (match exp 
	 ...
	 (interp-cast v t1 t2 l next))]
      [(Cast (app recur/next e next) t1 t2 l)
       (specialize-cast e (Type t1) (Type t2) (Quote l))]
      [(Fn-Cast (app recur/next e next) t1 t2 l)
       (cast-Fn->Fn e (Type t1) (Type t2) (Quote l))]
...
(: specialize-cast Cast-Rule)
(define (specialize-cast v t1 t2 l next)
  ...
  (let$* ([type2 t2])
    (bind (next)
     (cond$ 
      ...
      [(op=? (Type DYN-TYPE) type2) (values (Dyn-make v INT-TYPE) next)]
      [else (values (Blame lbl) next)]))))
  ...
  (let$* ([type2 t2])
    (bind (next)
      (cond$ 
       ...
       [(op=? (Type DYN-TYPE) type2) (values (Dyn-make v BOOL-TYPE) next)]
       [else (values (Blame lbl) next)]))))
