...
(: label-lambdas (Cast-or-Coerce3-Lang Config  -> Cast-or-Coerce4-Lang))
...
(: ll-expr! ((Boxof Nat) -> (CoC3-Expr -> CoC4-Expr)))
(define ((ll-expr! next) exp)
  ...
  (: ll-let (CoC3-Bnd* CoC3-Expr -> CoC4-Expr))
  (define (ll-let b* e)
    ;; split-bound-procedures filters lambdas out of lets
    (: split-bnds (-> CoC3-Bnd* (values CoC4-Bnd-Lambda* CoC4-Bnd-Data*)))
    (define (split-bnds b*)
  ...
  (: ll-bndc  (CoC3-Bnd-Code -> CoC4-Bnd-Code))
  (define (ll-bndc u.c)
...
;;TODO delete this block once the rest of the code is working
#|
;; And all the boring cases
        [(If tst csq alt)
