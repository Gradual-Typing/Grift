...
(define (interpret-casts/hyper-coercions)
  ...
  (define (apply-coercion e c)
    (apply-code apply-coercion-uid e c))
  (: apply-med-coercion : Apply-Coercion-Type)
  (define (apply-med-coercion e c)
    (apply-code apply-med-coercion-uid e c))
...
(define (make-apply-coercion-runtime!
  ...
  (add-cast-runtime-binding!
   ...
   (code$ (v c)
     (let*$ ([v1 (cond$
                  [(HC-Project-Huh c)
                   (project v (HC-T1 c) (HC-Label c))] 
                  [else v])]
             [m (HC-Med c)]
             [v2 (If (Id-Coercion-Huh m)
                     v1
                     (apply-med-coercion v1 m))])
       (If (HC-Inject-Huh c)
       ...
       (cond$
        ...
        [(Mediating-Coercion-Huh m)
         (cond$
          ...
          [(Tuple-Coercion-Huh m) (apply-tup-coercion v m)]
