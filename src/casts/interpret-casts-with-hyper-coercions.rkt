  ...
  (define (recur e)
    ...
    (match e
      ...
      [(MBoxCastedRef addr t)
       (define t1 (next-uid! "t1"))
       (define t2 (next-uid! "t2"))
       (define c  (next-uid! "crcn"))
       ;; Here we are passing an empty blame label because monotonic
       ;; references aren't fully implemented.
       (define ce (interp-make-coercion (Var t1) (Var t2) (Quote "")))
      ...
      [(MBoxCastedSet! addr (app recur e) t)
       (define (mbox-set! [addr : Uid]
                          [val : (U (Quote Cast-Literal) (Var Uid))]
                          [t1 : (Var Uid)])
         : CoC3-Expr
         (let$* ([t2    (Mbox-rtti-ref addr)]
                 ;; Here we are passing an empty blame label because
                 ;; we haven't implemented blame tracking for monotonic
                 ;; yet.
                 [crcn  (interp-make-coercion t1 t2 (Quote ""))]
                 ;; If the runtime type and the static type
                 ;; of the monotonic value are both tuple types
                 ;; then we have to copy the tuple because...
                 ;; TODO Deya please finish commenting this code. 
                 [cv (cond$
                      [(and$ (tupleT?$ t1) (tupleT?$ t2))
                       (let$* ([n (Type-Tuple-num t2)]
                               [ctv (Copy-Tuple n val)])
                        (Begin (list (Mbox-val-set! (Var addr) ctv))
                               ctv))]
                      [else val])]
                 [ccv (interp-cast cv crcn (Var addr))]
                 [t2-new (Mbox-rtti-ref addr)])
           (if$ (op=? t2 t2-new)
                (Mbox-val-set! (Var addr) ccv)
                ;; Once again why are we returning 0?
                ;; This seems like it should be the unit value
                (Quote 0))))
       (define t1 (next-uid! "t1"))
       (Let `([,t1 . ,(Type t)])
        (if (or (Var? e) (Quote? e))
            (mbox-set! addr e (Var t1))
            (let ([v (next-uid! "write_cv")])
              (Let `([,v . ,e])
                   (mbox-set! addr (Var v) (Var t1))))))]
      [(MVectCastedRef addr (app recur i) t)
       ...
       (Let `([,t1 . ,(Mvector-rtti-ref addr)]
              [,t2 . ,(Type t)])
            ;; Here we use a empty blame label because we haven't
            ;; implemented blame for monotonic references.
        (Let `([,c . ,(interp-make-coercion (Var t1) (Var t2) (Quote ""))])
         (interp-cast (Mvector-val-ref (Var addr) i) (Var c))))]
      [(MVectCastedSet! addr (app recur i) (app recur e) t)
       (define (mvect-set! [addr : Uid] [i : CoC3-Expr]
                           [val : (U (Quote Cast-Literal) (Var Uid))]
                           [t1 : (Var Uid)])
         : CoC3-Expr
         (let$* ([t2 (Mvector-rtti-ref addr)]
                 ;; Here we are using an empty blame label because monotonic
                 ;; references haven't been fully implemented.                  
      ...
      ;; Cases of Dyn-Ops that are currently lacking in other implementations
          ...
          [(MRef-Coercion-Huh m)
           (let*$ ([t2 (MRef-Coercion-Type m)])
             (cond$
              [(Type-Dyn-Huh t2) v]
              [else
               (let*$ ([t1 (Mbox-rtti-ref (Var-id v))]
                       [t3 (greatest-lower-bound t1 t2)])
                 (cond$
                  [(op=? t1 t3) v]
                  [else
                   (Mbox-rtti-set! (Var-id v) t3)
          ...
          [(MVect-Coercion-Huh m)
           (let$* ([t2 (MVect-Coercion-Type m)])
             (cond$
              [(Type-Dyn-Huh t2) v]
              [else
               (let*$ ([t1 (Mvector-rtti-ref (Var-id v))]
                       [t3 (greatest-lower-bound t1 t2)])
                 (cond$
                  [(op=? t1 t3) v]
                  [else
                   (Mvector-rtti-set! (Var-id v) t3)
...
(define (make-dynamic-operations-helpers!
      ...
      [((Var _) t1 (Type (Dyn)))
       (If (Type-Dyn-Huh t1)
           ...
           (interp-cast val (HC (Quote #f) t1 (Quote #f) (Quote #t) t1
                                (Quote-Coercion IDENTITY))))]
      [(val t1 t2)
       (interp-cast val (interp-make-coercion t1 t2 lbl))]))
    ...
    (Let `((,val . ,(Dyn-value dyn))
         ...
         (If (Type-GRef-Huh var-ty)
             (Let `([,tyof . ,(Type-GRef-Of var-ty)]
                  ...
                  (smart-cast var-read-val var-tyof (Type DYN-TYPE) lbl))
             (Blame lbl))))
      ...
      (If (Type-GRef-Huh ty)
          (let$ ([tyof (Type-GRef-Of ty)])
            (cond$
             ...
             [else
              (let$ ([wrt-val2 (smart-cast wrt-val1 t2 tyof lbl)])
                (pbox-set! gbox wrt-val2))])
            (smart-cast (Quote '()) (Type UNIT-TYPE) dyn-type lbl))
          (Blame lbl))))
      ...
      (cond$
       ...
       [else
        (let$ ([elem-ty (Type-GVect-Of maybe-pvec-ty)]
          ...
          (smart-cast elem-val elem-ty dyn-type lbl))])))
       ...
       [else
        (let*$ ([elem-type (Type-GVect-Of maybe-vec-type)]
                [new-elem (If (op=? elem-type t2)
                              wrt-val1
                              (smart-cast wrt-val1 t2 elem-type lbl))])
    ...
    (define arg-casts : CoC3-Expr*
      (for/list : (Listof CoC3-Expr)
        ...
        (let$ ([dyn-fn-arg-type (Type-Fn-arg (Var ty) (Quote i))])
          (smart-cast v (Type t) dyn-fn-arg-type l))))
    ...
    (Let `([,val . ,(Dyn-value v)]
     ...
     (If (Type-Fn-Huh (Var ty))
         (Let `([,ret-val . ,casts-apply]
           ...
           (smart-cast (Var ret-val) (Var ret-ty) (Type DYN-TYPE) l))
         (Blame l))))
          ...
          (cond$
           ...
           [else
            (ann (let$ ([prj-val (Tuple-proj val i)]
                   ...
                   (smart-cast prj-val prj-ty (Type DYN-TYPE) l))
                 CoC3-Expr)])))
  ...
  (case (dynamic-operations?)
    ...
    [else
     ...
     (define dpvl   (next-uid! "rt_dyn_pvec_set"))
