...
(: iic-expr ((Boxof Nat) S1-Expr -> C0-Expr))
(define (iic-expr next exp^)
    ...
    (match-let ([(Ann exp (cons src type)) exp^])
      (match exp
        ...
        [(Let bnd* (and (Ann _ (cons src type^)) body))
         (Let (map iic-bnd bnd*) (mk-cast (mk-label "let" src) (recur body) type^ type))]
        ...
        [(Repeat index
                 (and (Ann _ (cons s1 t1)) (app recur e1))
                 (and (Ann _ (cons s2 t2)) (app recur e2))
                 acc
                 (and (Ann _ (cons s3 t3)) (app recur e3))
                 (and (Ann _ (cons s4 t4)) (app recur e4)))
         (Repeat
          index
        ...
        [(Tuple-proj (and (Ann _ (cons e-src e-ty)) (app recur e)) i)
         (cond
           [(Dyn? e-ty)
            (let ([n (+ i 1)])
              (unless (index? n) (error 'recur "bad index"))
